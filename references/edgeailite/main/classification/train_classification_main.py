...
parser.add_argument('--exp_name', type=str, default="EXP_v0", help='experiment name')
...
#args.quantize = True
#args.phase = 'training'
#args.opset_version = 9					# Try opset 9 for TIDL onnx
...
# name an experiment
args.exp_name = cmds.exp_name

################################
for key in vars(cmds):
	if key in ['gpus', 'exp_name']: # already taken care above, since this has to be done before importing pytorch
		pass
...
if 'training' in args.phase and (not args.quantize):
	...
	args.scheduler = 'poly'
	args.polystep_power = 5
	train_classification.main(args)
	# --quantize True --lr 1e-5 --epochs 10 --warmup_epochs 0 --evaluate_start False
